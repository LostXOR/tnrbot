...
# Load JSON database from disk and initialize the bot
...
async def leaderboard(
    ...
    if page is None:
        page = 1
    maxPages = -(-len(users) // config.pageSize)
    page = max(min(page, maxPages), 1)

    # Get the highest players up to the right page and chop off the unneeded pages before
